...
public abstract class XYChart extends AbstractChart {
	...
	public void draw(Canvas canvas, int x, int y, int width, int height,
		...
		int legendSize = getLegendSize(mRenderer, height / 5,
				mRenderer.getAxisTitleTextSize());
		...
		for (int i = 0; i < sLength; i++) {
			maxScaleNumber = Math.max(maxScaleNumber, mDataset.getSeriesAt(i)
					.getScaleNumber());
		}
		maxScaleNumber++;
		if (maxScaleNumber < 0) {
		...
		for (int i = 0; i < sLength; i++) {
			...
			int scale = series.getScaleNumber(); //¿Ì¶È
		...
		for (int i = 0; i < sLength; i++) {
			...
			float yAxisValue = Math.min(bottom,
					(float) (bottom + yPixelsPerUnit[scale] * minY[scale]));
			LinkedList<ClickableArea> clickableArea = new LinkedList<ClickableArea>();

			clickableAreas.put(i, clickableArea);

			SortedMap<Double, Double> range = series.getRange(minX[scale],
					maxX[scale], 1);
			int startIndex = -1;
			...
			for (Entry<Double, Double> value : range.entrySet()) {
				...
				if (!isNullValue(yValue)) {
					...
					if (points.size() > 0) {
						drawSeries(series, canvas, paint, points,
								seriesRenderer, yAxisValue, i, or, startIndex);
						ClickableArea[] clickableAreasForSubSeries = clickableAreasForPoints(
								MathHelper.getFloats(points),
								MathHelper.getDoubles(values), yAxisValue, i,
								startIndex);
						clickableArea.addAll(Arrays
								.asList(clickableAreasForSubSeries));
						points.clear();
						values.clear();
					}
			...
			if (points.size() > 0) {
				drawSeries(series, canvas, paint, points, seriesRenderer,
						yAxisValue, i, or, startIndex);
				ClickableArea[] clickableAreasForSubSeries = clickableAreasForPoints(
						MathHelper.getFloats(points),
						MathHelper.getDoubles(values), yAxisValue, i,
						startIndex);
		...
		if (showLabels || showGridX) {
			List<Double> xLabels = getValidLabels(getXLabels(minX[0], maxX[0],
					mRenderer.getXLabels()));
			Map<Integer, List<Double>> allYLabels = getYLabels(minY, maxY,
					maxScaleNumber);
			...
			drawXLabels(xLabels, mRenderer.getXTextLabelLocations(), canvas,
					paint, xLabelsLeft, top, bottom, xPixelsPerUnit[0],
					minX[0], maxX[0]);
			drawYLabels(allYLabels, canvas, paint, maxScaleNumber, left, right,
					bottom, yPixelsPerUnit, minY);

			if (showLabels) {
				paint.setColor(mRenderer.getLabelsColor());
				for (int i = 0; i < maxScaleNumber; i++) {
					Align axisAlign = mRenderer.getYAxisAlign(i);
					Double[] yTextLabelLocations = mRenderer
							.getYTextLabelLocations(i);
							...
							if (or == Orientation.HORIZONTAL) {
								if (axisAlign == Align.LEFT) {
									canvas.drawLine(left
											+ getLabelLinePos(axisAlign),
											yLabel, left, yLabel, paint);
									drawText(canvas, label, left, yLabel - 2,
											paint, mRenderer.getYLabelsAngle());
								} else {
									canvas.drawLine(right, yLabel, right
											+ getLabelLinePos(axisAlign),
											yLabel, paint);
									drawText(canvas, label, right, yLabel - 2,
											paint, mRenderer.getYLabelsAngle());
								}

								if (showCustomTextGrid) {
									paint.setColor(mRenderer.getGridColor());
									canvas.drawLine(left, yLabel, right,
											yLabel, paint);
								}
							} else {
								canvas.drawLine(right
										- getLabelLinePos(axisAlign), yLabel,
										right, yLabel, paint);
								drawText(canvas, label, right + 10, yLabel - 2,
										paint, mRenderer.getYLabelsAngle());
								if (showCustomTextGrid) {
									paint.setColor(mRenderer.getGridColor());
									canvas.drawLine(right, yLabel, left,
											yLabel, paint);
								}
							}
						}
			...
			if (showLabels) {
				...
				if (or == Orientation.HORIZONTAL) {
					...
					for (int i = 0; i < maxScaleNumber; i++) {
						Align axisAlign = mRenderer.getYAxisAlign(i);
						if (axisAlign == Align.LEFT) {
							drawText(canvas, mRenderer.getYTitle(i), x + size,
									y + height / 2, paint, -90);
						} else {
							drawText(canvas, mRenderer.getYTitle(i), x + width,
									y + height / 2, paint, -90);
						}
					}
					...
					drawText(canvas, mRenderer.getXTitle(), x + width / 2, y
							+ height - size, paint, -90);
					drawText(canvas, mRenderer.getYTitle(), right + 20, y
							+ height / 2, paint, 0);
					paint.setTextSize(mRenderer.getChartTitleTextSize());
					drawText(canvas, mRenderer.getChartTitle(), x + size, top
							+ height / 2, paint, 0);
				}
			}
		}
	...
	protected Map<Integer, List<Double>> getYLabels(double[] minY,
			double[] maxY, int maxScaleNumber) {
		Map<Integer, List<Double>> allYLabels = new HashMap<Integer, List<Double>>();
		for (int i = 0; i < maxScaleNumber; i++) {
			allYLabels.put(i, getValidLabels(MathHelper.getLabels(minY[i],
					maxY[i], mRenderer.getYLabels())));
		}
		return allYLabels;
	}
	...
	protected void drawSeries(XYSeries series, Canvas canvas, Paint paint,
		...
		if (stroke != null) {
			...
			if (stroke.getIntervals() != null) {
				effect = new DashPathEffect(stroke.getIntervals(),
						stroke.getPhase());
			}
			setStroke(stroke.getCap(), stroke.getJoin(), stroke.getMiter(),
					Style.FILL_AND_STROKE, effect, paint);
		}
		...
		if (isRenderPoints(seriesRenderer)) {
			...
			if (pointsChart != null) {
				pointsChart.drawSeries(canvas, paint, points, seriesRenderer,
						yAxisValue, seriesIndex, startIndex);
			}
		}
		...
		if (seriesRenderer.isDisplayChartValues()) {
			...
			drawChartValuesText(canvas, series, seriesRenderer, paint, points,
					seriesIndex, startIndex);
		}
		if (stroke != null) {
	...
	private void setStroke(Cap cap, Join join, float miter, Style style,
			PathEffect pathEffect, Paint paint) {
	...
	protected void drawChartValuesText(Canvas canvas, XYSeries series,
			SimpleSeriesRenderer renderer, Paint paint, float[] points,
		...
		if (points.length > 1) { // there are more than one point
			...
			for (int k = 0; k < points.length; k += 2) {
				if (k == 2) { // decide whether to display first two points'
								// values or
								// not
					if (Math.abs(points[2] - points[0]) > renderer
							.getDisplayChartValuesDistance()
							|| Math.abs(points[3] - points[1]) > renderer
									.getDisplayChartValuesDistance()) {
						// first point
						drawText(canvas, getLabel(series.getY(startIndex)),
								points[0],
								points[1] - renderer.getChartValuesSpacing(),
								paint, 0);
						// second point
						drawText(canvas, getLabel(series.getY(startIndex + 1)),
								points[2],
								points[3] - renderer.getChartValuesSpacing(),
								paint, 0);
					...
					if (Math.abs(points[k] - previousPointX) > renderer
							.getDisplayChartValuesDistance()
							|| Math.abs(points[k + 1] - previousPointY) > renderer
									.getDisplayChartValuesDistance()) {
						drawText(
								canvas,
								getLabel(series.getY(startIndex + k / 2)),
								points[k],
								points[k + 1]
			...
			for (int k = 0; k < points.length; k += 2) {
				drawText(canvas, getLabel(series.getY(startIndex + k / 2)),
						points[k],
						points[k + 1] - renderer.getChartValuesSpacing(),
						paint, 0);
			}
		}
	}
	...
	protected void drawXLabels(List<Double> xLabels,
			...
			int top, int bottom, double xPixelsPerUnit, double minX, double maxX) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (showLabels) {
				paint.setColor(mRenderer.getXLabelsColor());
				canvas.drawLine(xLabel, bottom, xLabel,
						bottom + mRenderer.getLabelsTextSize() / 3, paint);
				drawText(canvas, getLabel(label), xLabel,
						bottom + mRenderer.getLabelsTextSize() * 4 / 3, paint,
		...
		drawXTextLabels(xTextLabelLocations, canvas, paint, showLabels, left,
					...
					if (showLabels && !textLabel) {
						...
						if (axisAlign == Align.LEFT) {
							canvas.drawLine(left + getLabelLinePos(axisAlign),
									yLabel, left, yLabel, paint);
							drawText(canvas, getLabel(label), left, yLabel - 2,
									paint, mRenderer.getYLabelsAngle());
						} else {
							canvas.drawLine(right, yLabel, right
									+ getLabelLinePos(axisAlign), yLabel, paint);
							drawText(canvas, getLabel(label), right,
									yLabel - 2, paint,
	...
	public double[] toRealPoint(float screenX, float screenY, int scale) {
		...
		return new double[] {
				(screenX - mScreenR.left) * (realMaxX - realMinX)
						/ mScreenR.width() + realMinX,
				(mScreenR.top + mScreenR.height() - screenY)
						* (realMaxY - realMinY) / mScreenR.height() + realMinY };
	}
	...
	public double[] toScreenPoint(double[] realPoint, int scale) {
		...
		if (!mRenderer.isMinXSet(scale) || !mRenderer.isMaxXSet(scale)
				|| !mRenderer.isMinXSet(scale) || !mRenderer.isMaxYSet(scale)) {
		...
		return new double[] {
				(realPoint[0] - realMinX) * mScreenR.width()
						/ (realMaxX - realMinX) + mScreenR.left,
				(realMaxY - realPoint[1]) * mScreenR.height()
						/ (realMaxY - realMinY) + mScreenR.top };
	}
			...
			for (int seriesIndex = clickableAreas.size() - 1; seriesIndex >= 0; seriesIndex--) {
				...
				if (clickableAreas.get(seriesIndex) != null) {
					...
					for (ClickableArea area : clickableAreas.get(seriesIndex)) {
						...
						if (rectangle != null
								&& rectangle.contains(screenPoint.getX(),
										screenPoint.getY())) {
							return new SeriesSelection(seriesIndex, pointIndex,
									area.getX(), area.getY());
						}
						pointIndex++;
					}
	 ...
	 *            the start index of the rendering points
	 */
	protected abstract ClickableArea[] clickableAreasForPoints(float[] points,
