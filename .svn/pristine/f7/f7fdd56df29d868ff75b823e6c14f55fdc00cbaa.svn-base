...
public class BarChart extends XYChart {
	...
	protected ClickableArea[] clickableAreasForPoints(float[] points,
		...
		for (int i = 0; i < length; i += 2) {
			...
			if (mType == Type.STACKED) {
				ret[i / 2] = new ClickableArea(new RectF(x - halfDiffX, y, x
						+ halfDiffX, yAxisValue), values[i], values[i + 1]);
			} else {
				float startX = x - seriesNr * halfDiffX + seriesIndex * 2
						* halfDiffX;
				ret[i / 2] = new ClickableArea(new RectF(startX, y, startX + 2
						* halfDiffX, yAxisValue), values[i], values[i + 1]);
			}
		}
		return ret;
	 ...
	 *            the paint
	 */
	private void drawBar(Canvas canvas, float xMin, float yMin, float xMax,
			float yMax, int scale, int seriesIndex, Paint paint) {
		SimpleSeriesRenderer renderer = mRenderer
				.getSeriesRendererAt(seriesIndex);
		if (renderer.isGradientEnabled()) {
			float minY = (float) toScreenPoint(
					new double[] { 0, renderer.getGradientStopValue() }, scale)[1];
			float maxY = (float) toScreenPoint(
					new double[] { 0, renderer.getGradientStartValue() }, scale)[1];
			...
			if (yMin < minY) {
				...
				canvas.drawRect(Math.round(xMin), Math.round(yMin),
						Math.round(xMax), Math.round(gradientMinY), paint);
			} else {
				gradientStopColor = getGradientPartialColor(gradientMinColor,
						gradientMaxColor, (maxY - gradientMinY) / (maxY - minY));
			}
			if (yMax > maxY) {
				paint.setColor(gradientMaxColor);
				canvas.drawRect(Math.round(xMin), Math.round(gradientMaxY),
						Math.round(xMax), Math.round(yMax), paint);
			} else {
				gradientStartColor = getGradientPartialColor(gradientMaxColor,
						gradientMinColor, (gradientMaxY - minY) / (maxY - minY));
			}
			GradientDrawable gradient = new GradientDrawable(
					Orientation.BOTTOM_TOP, new int[] { gradientStartColor,
							gradientStopColor });
			gradient.setBounds(Math.round(xMin), Math.round(gradientMinY),
					Math.round(xMax), Math.round(gradientMaxY));
			...
			if (temp < 150)
				...
				paint.setColor(Color.rgb(231, 101, 26));
			else
	...
	private int getGradientPartialColor(int minColor, int maxColor,
			float fraction) {
		int alpha = Math.round(fraction * Color.alpha(minColor)
				+ (1 - fraction) * Color.alpha(maxColor));
		int r = Math.round(fraction * Color.red(minColor) + (1 - fraction)
				* Color.red(maxColor));
	...
	protected void drawChartValuesText(Canvas canvas, XYSeries series,
		...
		for (int i = 0; i < points.length; i += 2) {
			...
			if (!isNullValue(value)) {
				...
				if (value >= 0) {
					drawText(canvas, getLabel(value), x, points[i + 1]
							- renderer.getChartValuesSpacing(), paint, 0);
				} else {
					drawText(canvas, getLabel(value), x,
							points[i + 1] + renderer.getChartValuesTextSize()
									+ renderer.getChartValuesSpacing() - 3,
							paint, 0);
				}
			}
		}
